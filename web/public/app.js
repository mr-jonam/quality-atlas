...
const state = { Notizen: [], filtered: [], active: null };
const elements = {
...
function noteTarget(raw, currentId) {
  ...
  if (!state.Notizen.some((note) => note.id === id)) {
    const match = state.Notizen.find((note) => note.id.endsWith(`/${clean}`) || note.id.endsWith(`/${clean.replace(/^.*\//, "")}`));
...
function unique(field) {
  return [...new Set(state.Notizen.flatMap((note) => Array.isArray(note[field]) ? note[field] : [note[field]]))].filter(Boolean).sort();
}
...
function applyFilters() {
  ...
  state.filtered = state.Notizen.filter((note) => {
...
function renderNavigation() {
  elements.count.textContent = `${state.filtered.length} of ${state.Notizen.length} Notizen`;
  if (!state.filtered.length) {
    elements.navigation.innerHTML = '<div class="empty-state"><strong>No matching Notizen</strong><span>Verwende weniger Wörter oder entferne einen Filter.</span><button type="button" id="clear-filters">Filter löschen</button></div>';
  ...
  elements.navigation.innerHTML = [...groups.entries()].map(([group, Notizen]) => `
    <section class="nav-group">
      <h2>${escapeHtml(groupLabel(group))}</h2>
      ${Notizen.map((note) => `<a href="#/doc/${note.id}" data-id="${note.id}" class="${note.id === state.active?.id ? "active" : ""}"><span>${escapeHtml(note.title)}</span><small>${escapeHtml(note.level)}</small></a>`).join("")}
    </section>`).join("");
}

function showNote(id) {
  const note = state.Notizen.find((item) => item.id === id) || state.Notizen.find((item) => item.id.endsWith("/home")) || state.Notizen[0];
  if (!note) return;
  state.active = note;
  document.title = `${note.title} | Quality Atlas`;
  elements.crumb.textContent = `${groupLabel(note.group)} / ${note.title}`;
  elements.source.href = `https://github.com/mr-jonam/quality-atlas/blob/lang/de/${note.path}`;
  elements.content.innerHTML = `
    <article class="note">
      <div class="note-meta"><span>${escapeHtml(note.level)}</span><time datetime="${escapeHtml(note.updated)}">Aktualisiert ${escapeHtml(note.updated)}</time></div>
...
async function initialize() {
  try {
    ...
    state.Notizen = (await response.json()).Notizen;
    state.filtered = state.Notizen;
    fillFilters(); applyFilters(); route();
  } catch (error) {
    elements.navigation.innerHTML = "";
    elements.content.innerHTML = `<div class="error-state"><strong>Der Atlas konnte nicht geladen werden.</strong><span>${escapeHtml(error.message)}</span><button type="button" id="retry-load">Erneut versuchen</button></div>`;
    document.querySelector("#retry-load").addEventListener("click", () => location.reload());
    elements.count.textContent = "Nicht verfügbar";
  }
}
...
document.querySelector("#copy-link").addEventListener("click", async () => {
  await navigator.clipboard.writeText(location.href); toast("Link kopiert");
});
